                    ...
                    if content_list[idx][id_input][0] != []:
                        if type(content_list[idx][id_input][idy]) == type([]):
                            for feature in content_list[idx][id_input][idy]:
                                inputs.append(feature.encode('utf-8').decode())
                        else:
                            inputs.append(content_list[idx][id_input][idy].encode('utf-8').decode())

                outputs = []
                for task in predict_results:
                    outputs.append(task[idx][idy])

                fout.write("\t".join(map(str, inputs)) + "\t" + "{}".join(map(str, outputs)) + '\n')
            fout.write('\n')
        fout.close()
    ...
    def read_config(self, config_file):
        ...
        if the_item in config:
            ...
            if self.HP_tasks > 1:
                ...
                self.label_alphabet_sizes = {idtask: self.label_alphabet[idtask].size() for idtask in
                                             range(self.HP_tasks)}

        the_item = 'tasks_weights'
        if the_item in config:
            self.HP_tasks_weights = list(map(float, config[the_item].split("|")))
        ...
        the_item = "raw_dev_dep"
        if gold:
            if the_item in config:
                self.raw_dev_dep = config[the_item]
            else:
                self.raw_dev_dep = self.gold_dev_dep

        the_item = "index_of_main_tasks"
        if the_item in config:
            self.index_of_main_tasks = list(map(int, config[the_item].split(",")))

        the_item = "encoding"
        if the_item in config:
            self.encoding = config[the_item]
